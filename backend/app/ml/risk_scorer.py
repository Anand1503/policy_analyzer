...
def _build_explanation(
    ...
    level_intros = {
        "low": f"This {cat_display} clause appears to be standard and poses minimal concern.",
        "medium": f"This {cat_display} clause has some potentially concerning elements worth noting.",
        "high": f"This {cat_display} clause contains language that may put users at a disadvantage.",
        "critical": f"This {cat_display} clause raises serious concerns and should be reviewed carefully.",
    }
    intro = level_intros.get(risk_level.lower(), f"This {cat_display} clause requires review.")

    if not factors:
        return intro

    friendly = [f.replace("_", " ").lower() for f in factors[:3]]
    return f"{intro} It involves: {', '.join(friendly)}."
