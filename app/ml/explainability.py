...
def _generate_justification(
    ...
    """Generate a deterministic, audit-friendly natural language justification."""
    parts = []

    # Risk level statement
    level_map = {
        "CRITICAL": "poses critical risk requiring immediate attention",
        "HIGH": "presents elevated risk that warrants careful review",
        "MEDIUM": "carries moderate risk that should be monitored",
        "LOW": "presents acceptable risk within normal parameters",
    }
    parts.append(f"This {category or 'policy'} clause {level_map.get(risk_level.upper(), 'requires review')}.")

    # Score breakdown
    base = explanation.get("base_risk", 0)
    entity = explanation.get("entity_bonus", 0)
    pattern = explanation.get("pattern_bonus", 0)

    if base > 0.3:
        parts.append(f"Category-inherent risk is significant (base: {base:.2f}).")
    if entity > 0:
        parts.append(f"Entity sensitivity contributes +{entity:.2f} to the risk score.")
    if pattern > 0:
        parts.append(f"Risk pattern matches add +{pattern:.2f}.")

    # Risk factors
    if risk_factors:
        factor_list = ", ".join(risk_factors[:5])
        parts.append(f"Key risk factors: {factor_list}.")

    # Entity types
    if entity_types:
        parts.append(f"Sensitive data types detected: {', '.join(entity_types[:5])}.")
...
def generate_explanations_batch(
    ...
    for clause in clauses:
        ...
        # Classification explanation (SHAP with fallback)
        cls_explanation = explain_classification_shap(
